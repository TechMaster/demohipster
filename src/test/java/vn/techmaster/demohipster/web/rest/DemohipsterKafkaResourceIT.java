...
import vn.techmaster.demohipster.config.KafkaProperties;

class DemohipsterKafkaResourceIT {
    ...
    void setup() {
        ...
        DemohipsterKafkaResource kafkaResource = new DemohipsterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/demohipster-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/demohipster-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
